 ...
 * @return {RC_VALUES}
 */
RC_VALUES SmManager::create_table_rc(const std::string &tab_name,
                                     const std::vector<ColDef> &col_defs,
                                     Context *context) {
  if (db_.is_table(tab_name)) {
    return RC_VALUES::RC_SM_MANAGER_EXISTS_TABLE;
  }
  // Create table meta
  int curr_offset = 0;
  TabMeta tab;
  ...
  auto rc = flush_meta_rc();
  if (rc != RC_VALUES::RC_SUCCESS) {
    LOG_WARNING("Error in File: %s, Line: %d", __FILE__, __LINE__);
    return rc;
  }
  return RC_VALUES::RC_SUCCESS;
}
 ...
 * @return{RC_VALUES}
 */
RC_VALUES SmManager::drop_table_rc(const std::string &tab_name,
                                   Context *context) {
  if (!db_.is_table(tab_name)) {
    return RC_VALUES::RC_SM_MANAGER_NO_TABLE;
  }
  auto file_handle = fhs_[tab_name].get();
  // TODO: 重构rm_manager之后要修改
  //  关闭file
  rm_manager_->close_file(file_handle);
  // 删除记录
  rm_manager_->destroy_file(tab_name);
...
void SmManager::create_index(const std::string &tab_name,
  ...
  // 放入ihs_
  ihs_.emplace(std::pair{std::move(index_name), std::move(open_index)});
  // 更新索引
 ...
 * @return {RC_VALUES}
 */
RC_VALUES SmManager::create_index_rc(const std::string &tab_name,
                                     const std::vector<std::string> &col_names,
                                     Context *context) {
  if (ix_manager_->exists(tab_name, col_names)) {
    return RC_VALUES::RC_SM_MANAGER_EXISTS_INDEX;
  }
  std::vector<ColMeta> index_columns;
  std::for_each(col_names.begin(), col_names.end(),
                [&index_columns, &tab_name, this](auto &col_name) {
 ...
 * @return {RC_VALUES}
 */
RC_VALUES SmManager::drop_index_rc(const std::string &tab_name,
                                   const std::vector<std::string> &col_names,
                                   Context *context) {
  try {
    // create_index逆向操作
