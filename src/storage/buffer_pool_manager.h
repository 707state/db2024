...
private:
  ...
  std::unordered_map<PageId, rmdb::frame_id_t, PageIdHash>
      page_table_; // 帧号和页面号的映射哈希表，用于根据页面的PageId定位该页面的帧编号
  std::list<rmdb::frame_id_t> free_list_; // 空闲帧编号的链表
  ...
  BufferPoolManager(size_t pool_size, DiskManager *disk_manager)
    ...
    for (size_t i = 0; i < pool_size_; ++i) {
      free_list_.emplace_back(
          static_cast<rmdb::frame_id_t>(i)); // static_cast转换数据类型
    }
  }
...
public:
  ...
  RC_VALUES flush_all_pages_rc(int fd);
  void flush_all_pages(int fd);

private:
  bool find_victim_page(rmdb::frame_id_t *frame_id);
  RC_VALUES update_page_rc(Page *page, PageId new_page_id,
                           rmdb::frame_id_t new_frame_id);
  void update_page(Page *page, PageId new_page_id,
                   rmdb::frame_id_t new_frame_id);
};
