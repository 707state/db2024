...
public:
  explicit FileNotClosedError(const std::string &filename)
      : RMDBError("File is opened: " + filename) {}
};

class FileExistsError : public RMDBError {
public:
  explicit FileExistsError(const std::string &filename)
...
public:
  explicit InvalidColLengthError(int col_len)
...
public:
  explicit AmbiguousColumnError(const std::string &col_name)
