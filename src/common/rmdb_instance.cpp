...
#include "common/rmdb_instance.h"
...
auto rmdbInstance::MakeExecutorContext(Transaction *txn)
...
rmdbInstance::rmdbInstance(const std::string &db_file_name) {
  ...
  try {
    buffer_pool_manager_ = new BufferPoolManagerInstance(
        128, disk_manager_, LRUK_REPLACER_K, log_manager_);
...
void rmdbInstance::CmdDisplayTables(ResultWriter &writer) {
...
void rmdbInstance::CmdDisplayIndices(ResultWriter &writer) {
  auto table_names = catalog_->GetTableNames();
  writer.BeginTable(false);
  writer.BeginHeader();
...
void rmdbInstance::WriteOneCell(const std::string &cell, ResultWriter &writer) {
...
void rmdbInstance::CmdDisplayHelp(ResultWriter &writer) {
  std::string help = R"(Welcome to the rmdb shell!

\dt: show all tables
\di: show all indices
\help: show this message again

rmdb shell currently only supports a small set of Postgres queries. We'll set
...
auto rmdbInstance::ExecuteSql(const std::string &sql,
                              ResultWriter &writer) -> bool {
...
auto rmdbInstance::ExecuteSqlTxn(const std::string &sql, ResultWriter &writer,
  ...
  for (auto *stmt : binder.statement_nodes_) {
    ...
    case StatementType::CREATE_STATEMENT: {
      ...
      auto info = catalog_->CreateTable(txn, create_stmt.table_,
                                        Schema(create_stmt.columns_));
      l.unlock();

      if (info == nullptr) {
        throw rmdb::Exception("Failed to create table");
      }
      WriteOneCell(fmt::format("Table created with id = {}", info->oid_),
                   writer);
      continue;
    }
    case StatementType::INDEX_STATEMENT: {
      ...
      for (const auto &col : index_stmt.cols_) {
        ...
        if (index_stmt.table_->schema_.GetColumn(idx).GetType() !=
            TypeId::INTEGER) {
      ...
      if (col_ids.size() != 1) {
        throw NotImplementedException(
            "only support creating index with exactly one column");
      }
      auto key_schema =
          Schema::CopySchema(&index_stmt.table_->schema_, col_ids);

      std::unique_lock<std::shared_mutex> l(catalog_lock_);
      auto info = catalog_->CreateIndex<IntegerKeyType, IntegerValueType,
                                        IntegerComparatorType>(
          txn, index_stmt.index_name_, index_stmt.table_->table_,
          index_stmt.table_->schema_, key_schema, col_ids, INTEGER_SIZE,
          IntegerHashFunctionType{});
      l.unlock();

      if (info == nullptr) {
        throw rmdb::Exception("Failed to create index");
      }
    ...
    case StatementType::EXPLAIN_STATEMENT: {
      ...
      rmdb::Optimizer optimizer(*catalog_, IsForceStarterRule());
...
/**
 * FOR TEST ONLY. Generate test tables in this rmdb instance.
 ...
 */
void rmdbInstance::GenerateTestTable() {
  auto txn = txn_manager_->Begin();
  auto exec_ctx = MakeExecutorContext(txn);
 ...
 * create / drop table and insert for now. Should remove it in the future.
 */
void rmdbInstance::GenerateMockTable() {
