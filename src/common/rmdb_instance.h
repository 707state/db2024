...
// rmdb_instance.h
//
// Identification: src/include/common/rmdb_instance.h
//
...
private:
  /**
   * Get the executor context from the rmdb instance.
   */
  auto
  MakeExecutorContext(Transaction *txn) -> std::unique_ptr<ExecutorContext>;

public:
  explicit rmdbInstance(const std::string &db_file_name);
  ...
  /**
   * Execute a SQL query in the rmdb instance.
   */
  auto ExecuteSql(const std::string &sql, ResultWriter &writer) -> bool;

  /**
   * Execute a SQL query in the rmdb instance with provided txn.
   */
  ...
  /**
   * FOR TEST ONLY. Generate mock tables in this rmdb instance.
   * It's used in the shell to predefine some tables, as we don't support
   * create / drop table and insert for now. Should remove it in the future.
   */
