...
auto TinyintType::Subtract(const Value &left,
...
auto TinyintType::Multiply(const Value &left,
                           const Value &right) const -> Value {
  assert(left.CheckInteger());
  assert(left.CheckComparable(right));
  if (left.IsNull() || right.IsNull()) {
...
auto TinyintType::CompareEquals(const Value &left,
...
auto TinyintType::CompareNotEquals(const Value &left,
                                   const Value &right) const -> CmpBool {
  assert(left.CheckInteger());
  assert(left.CheckComparable(right));
  if (left.IsNull() || right.IsNull()) {
...
auto TinyintType::CompareLessThan(const Value &left,
                                  const Value &right) const -> CmpBool {
  assert(left.CheckInteger());
  assert(left.CheckComparable(right));
  if (left.IsNull() || right.IsNull()) {
...
auto TinyintType::CompareGreaterThanEquals(
...
auto TinyintType::CastAs(const Value &val,
