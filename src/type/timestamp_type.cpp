...
auto TimestampType::CompareEquals(const Value &left,
...
auto TimestampType::CompareLessThan(const Value &left,
                                    const Value &right) const -> CmpBool {
  assert(left.CheckComparable(right));
  if (left.IsNull() || right.IsNull()) {
    return CmpBool::CmpNull;
...
auto TimestampType::CompareGreaterThanEquals(
...
auto TimestampType::CastAs(const Value &val,
