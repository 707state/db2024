...
public:
  ...
  auto Subtract(const Value &left,
                const Value &right) const -> Value override = 0;
  auto Multiply(const Value &left,
                const Value &right) const -> Value override = 0;
  auto Divide(const Value &left,
              const Value &right) const -> Value override = 0;
  auto Modulo(const Value &left,
              const Value &right) const -> Value override = 0;
  ...
  auto CompareEquals(const Value &left,
                     const Value &right) const -> CmpBool override = 0;
  auto CompareNotEquals(const Value &left,
                        const Value &right) const -> CmpBool override = 0;
  auto CompareLessThan(const Value &left,
                       const Value &right) const -> CmpBool override = 0;
  auto CompareLessThanEquals(const Value &left,
                             const Value &right) const -> CmpBool override = 0;
  auto CompareGreaterThan(const Value &left,
                          const Value &right) const -> CmpBool override = 0;
...
protected:
  ...
  auto OperateNull(const Value &left,
                   const Value &right) const -> Value override = 0;
...
template <class T1, class T2>
auto IntegerParentType::AddValue(const Value &left,
...
template <class T1, class T2>
auto IntegerParentType::DivideValue(const Value &left,
                                    const Value &right) const -> Value {
  auto x = left.GetAs<T1>();
  auto y = right.GetAs<T2>();
  if (y == 0) {
...
template <class T1, class T2>
auto IntegerParentType::ModuloValue(const Value &left,
                                    const Value &right) const -> Value {
  auto x = left.GetAs<T1>();
  auto y = right.GetAs<T2>();
  if (y == 0) {
