...
auto DecimalType::Subtract(const Value &left,
...
auto DecimalType::Multiply(const Value &left,
                           const Value &right) const -> Value {
  assert(GetTypeId() == TypeId::DECIMAL);
  assert(left.CheckComparable(right));
  if (left.IsNull() || right.IsNull()) {
...
auto DecimalType::OperateNull(const Value &left __attribute__((unused)),
                              const Value &right
                              __attribute__((unused))) const -> Value {
  return {TypeId::DECIMAL, BUSTUB_DECIMAL_NULL};
}

auto DecimalType::CompareEquals(const Value &left,
...
auto DecimalType::CompareNotEquals(const Value &left,
                                   const Value &right) const -> CmpBool {
  assert(GetTypeId() == TypeId::DECIMAL);
  assert(left.CheckComparable(right));
  if (left.IsNull() || right.IsNull()) {
...
auto DecimalType::CompareLessThan(const Value &left,
                                  const Value &right) const -> CmpBool {
  assert(GetTypeId() == TypeId::DECIMAL);
  assert(left.CheckComparable(right));
  if (left.IsNull() || right.IsNull()) {
...
auto DecimalType::CompareGreaterThanEquals(
...
auto DecimalType::CastAs(const Value &val,
