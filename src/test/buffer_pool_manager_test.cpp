...
TEST_F(BufferPoolManagerTest, MultipleFilesTest) {
  ...
  for (auto &entry : fd2name) {
    ...
    for (int page_no = 0; page_no < MAX_PAGES; page_no++) {
      ...
      disk_manager_->read_page(fd, page_no, buf,
...
TEST_F(BufferPoolManagerTest, ConcurrencyTest) {
  ...
  for (int run = 0; run < num_runs; run++) {
    ...
    for (int j = 0; j < buffer_pool_size; j++) {
      ...
      strcpy(
          page->get_data(),
          (std::string("Hard") + std::to_string(page_ids[j].page_no)).c_str());
    }
      ...
      threads.push_back(std::thread([&bpm, tid, page_ids, fd]() {
        ...
        while (j < buffer_pool_size) {
          if (j != tid * 10) {
            ...
            EXPECT_EQ(0,
                      std::strcmp(std::to_string(temp_page_id.page_no).c_str(),
                                  (page_local->get_data())));
          ...
          if (j % 2 == 0) {
            EXPECT_EQ(0,
                      std::strcmp(std::to_string(page_ids[j].page_no).c_str(),
                                  (page->get_data())));
            EXPECT_EQ(true, bpm->unpin_page(page_ids[j], false));
          } else {
            EXPECT_EQ(0, std::strcmp((std::string("Hard") +
